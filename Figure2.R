...
s_d       <- 0.8 #per capita seedling death
reprod    <- merge(fert,pred_till)
reprod    <- mutate(reprod, reprod    = (viable_seeds * s_d) + pred_new_t)
reprod    <- mutate(reprod, reprod_pf = reprod /n_fem )
reprod    <- mutate(reprod, reprod_pc = reprod /TotDensity)
vs_3d_09   <- form_3d_surf(reprod,reprod_pc)
...
reprod    <- mutate(reprod, reprod_pf = reprod /n_fem )
reprod    <- mutate(reprod, reprod_pc = reprod /TotDensity)
vs_3d_08  <- form_3d_surf(reprod,reprod_pc)

s_d       <- 0.01 #per capita seedling death
reprod    <- merge(fert,pred_till)
reprod    <- mutate(reprod, reprod    = (viable_seeds * s_d) + pred_new_t)
reprod    <- mutate(reprod, reprod_pf = reprod /n_fem )
reprod    <- mutate(reprod, reprod_pc = reprod /TotDensity)
vs_3d_07  <- form_3d_surf(reprod,reprod_pc)


tiff("Results/VitalRates_3/figure2_sSurv.tiff",unit="in",width=6.3,height=6.3,res=600,compression="lzw")
...
persp(vs_3d_09$x,vs_3d_09$y,vs_3d_09$z, theta = 30, phi = 30, expand = 0.5, col = "lightblue",
      xlab = "Proportion of female individuals", line = m_line,
      ylab = "Density",ticktype = "detailed",
      zlab = "Regeneration",
      main = expression("b)   80% seedling surv"))
persp(vs_3d_08$x,vs_3d_08$y,vs_3d_08$z, theta = 30, phi = 30, expand = 0.5, col = "lightblue",
      xlab = "Proportion of female individuals", line = m_line,
      ylab = "Density",ticktype = "detailed",
      zlab = "Regeneration",
      main = expression("c)   5% seedling surv"))
persp(vs_3d_07$x,vs_3d_07$y,vs_3d_07$z, theta = 30, phi = 30, expand = 0.5, col = "lightblue",
      xlab = "Proportion of female individuals", 
      ylab = "Density",ticktype = "detailed",line = m_line,
      zlab = "Regeneration",
      main = expression("d)   1% seedling surv"))
