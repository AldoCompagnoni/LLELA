...
plot(fecund_data$TotDensity, fecund_data$SeedN, pch = 21, 
     bg = cRamp(fecund_data$sr) , ylim = c(0, 1000),
...
# Sex ratio as dot dimension ------------------------------------------------------
tiff("Results/VitalRates_3/figure1_a.tiff",unit="in",width=6.3,height=6.3,res=600,compression="lzw")
...
plot(fecund_data$TotDensity, fecund_data$SeedN, pch = 1, 
     cex = (fecund_data$sr+0.1) * 1.5 , ylim = c(0, 1000),
     xlab = "Planting density", ylab = "Seeds per flower")
xSeq   <- seq(0,48,by = 1)
mSize  <- mean(fecund_data$log_l_t0)
beta   <- fec_beta$avg
...
legend(20,1060,c("95% female plot","50% female plot", "  5% female plot"),
       pch = 1, pt.cex = (c(0.95,0.5,0.05)+0.1)*1.5, bty="n")
...
# Tetrazolium data
plot(jitter(viabVr$totFlow,factor = 2),jitter(viabVr$germ_ratio,factor = 2),pch=1,ylim=c(0,1.01), 
     cex = viabVr$sr_f * 1.5 ,
     xlab="Number of flowers",ylab="Seed viability rate")
