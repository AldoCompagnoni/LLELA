...
tiff("Results/VitalRates_3/figure1.tiff",unit="in", width=6.3,height=9.45,res=600,compression="lzw")
...
abline(h = y_m, lwd=2, col = "black")
abline(h = y_f, lty=2, lwd=2, col = "#ABABAB")

colfunc      <- colorRampPalette(cRamp(unique(arrange(d15,sr)$sr)))
legend_image <- as.raster(matrix(colfunc(21), ncol=1))
text(x=29, y = seq(-2.75,-1.25,l=3), labels = seq(0,1,l=3), cex = 1.3)
rasterImage(legend_image, 31.5, -2.75, 35.5, -1.25)
text(35, -1.25, "Percent of", pos = 4, cex = 1.3)
text(35, -2, "males in", pos = 4, cex = 1.3)
text(35, -2.75, "plot", pos = 4, cex = 1.3)

legend("bottomleft", c("Male","Female"), lty=c(1,2), lwd=2, cex = cex_leg, 
       col=c("black","#ABABAB"), bty="n", pch = c(17,16) )
...
plot(jitter(f14$TotDensity), jitter(f14$flowN_t1, factor = 3), 
     ...
     ylab = "Number of panicles per capita", xlab="Planting density")
...
legend("topleft", c("95% female plot","  5% female plot"),
...
plot(till14$TotDensity,till14$new_t1,pch=21,ylab="Number of new tillers",
