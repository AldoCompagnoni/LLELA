...
lr_14_avg       <- model_avg(AICtab(m14, weights = T), m14)
write.csv(lr_14_avg, "Results/VitalRates_3/log_ratio_14_best.csv", row.names = F)

# model selection results
x <- m14
mod_14_w        <- rgr_mod_sel(m14)
write.csv(mod_14_w,"Results/VitalRates_3/mod_sel_log_ratio_14.csv",row.names=F)
rm(x)
...
write.csv(mod_15_w,"Results/VitalRates_3/mod_sel_log_ratio_15.csv",row.names=F)
rm(x)