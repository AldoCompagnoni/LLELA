...
nt[[2]] <- lm(new_t1_pc ~ TotDensity + sr, data=d15)
#Interactions b/w density and sex ratio
nt[[3]] <- lm(new_t1_pc ~ sr * TotDensity, data=d15)
AICtab(nt,weights=T)
...
#Effect of density + sex ratio
nt[[2]] <- glmmadmb(new_t1 ~ TotDensity + sr, data=d15, family="nbinom2")
#Interactions b/w density and sex ratio
nt[[3]] <- glmmadmb(new_t1 ~ sr * TotDensity, data=d15, family="nbinom2")
...
#Effect of density + sex ratio
nt[[2]] <- lm(new_t1_pc ~ TotDensity + sr, data=d14)
#Interactions b/w density and sex ratio
nt[[3]] <- lm(new_t1_pc ~ sr * TotDensity, data=d14)
...
#Effect of density + sex ratio
nt[[2]] <- glmmadmb(new_t1 ~ TotDensity + sr, data=d14, family="nbinom2")
#Interactions b/w density and sex ratio
nt[[3]] <- glmmadmb(new_t1 ~ sr * TotDensity, data=d14, family="nbinom2")
...
tiff("Results/VitalRates_3/growth_newTillers_linear.tiff",unit="in",width=6.3,height=9.45,res=600,compression="lzw")
...
xSeq  <-  seq(0,48,by=1)
yL    <-  beta[1] + beta[2]*xSeq + beta[3]*0.1 + beta[4]*xSeq*0.1
yH    <-  beta[1] + beta[2]*xSeq + beta[3]*0.9 + beta[4]*xSeq*0.9
...
xSeq  <-  seq(0,48,by=1)
yL    <-  exp( beta[1] + beta[2]*xSeq + beta[3]*0.1 + beta[4]*xSeq*0.1 )
yH    <-  exp( beta[1] + beta[2]*xSeq + beta[3]*0.9 + beta[4]*xSeq*0.9 )
lines(xSeq,yL,col="red",lwd=2, lty = 2)
lines(xSeq,yH,col="blue",lwd=2)
title("2015")
